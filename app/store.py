...
class Store:
    ...
    def item(self, unique_key):
        ...
        return Item(events) if events else None
    ...
    def sync_ai_drafts(self, limit=500):
        ...
        sql = f'''
            SELECT source.*
            FROM `{self.ai_source_table}` AS source
            WHERE CAST(source.{key} AS STRING) IS NOT NULL
              AND CAST(source.{key} AS STRING) NOT IN (
