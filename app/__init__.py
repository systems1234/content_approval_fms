...
def _register_identity(app):
    """With the sign-in gate off, the app opens straight on the dashboard and
    you act as whoever is picked in the header. Turning REQUIRE_SIGNIN on puts
    Google SSO back in front of everything without touching anything else."""
    if app.config['REQUIRE_SIGNIN']:
        return

    @app.before_request
    def _identify():
        if current_user.is_authenticated or request.endpoint in (None, 'static'):
            return
        store = app.extensions['store']
        try:
            people = store.users(active_only=True)
        except Exception:
            return
        wanted = session.get('acting_as') or app.config['DEFAULT_ACTOR_EMAIL']
        user = next((p for p in people if p.email == wanted), None) or next(iter(people), None)
        if user:
