"""
Script to fix the corrupted task_detail.html template
This will remove the Content Requirements section from the main area
and add it to the sidebar as a collapsible section
"""

# Read the original template (before corruption)
# We'll reconstruct it properly

template_content = """{% extends "base.html" %}

{% block title %}{{ task.title }} - TaskFlow CRM{% endblock %}
                            ...
                            <div x-show="showNotes"
                                ...
                                <div x-show="selectedAction === 'complete'" class="mb-6 pb-6 border-b border-gray-200">
                                        ...
                                        <div class="flex gap-4 mt-2">
                                            ...
                                            <label class="inline-flex items-center cursor-pointer">
                                                <input type="radio"
                                                       ...
                                                       class="form-radio text-blue-600"
                                                       :required="selectedAction === 'complete'">
                                                <span class="ml-2">Google Sheet Link</span>
                                            </label>
                                        </div>
                                    </div>
                                    ...
                                    <div x-show="submissionType === 'sheet_link'" x-transition class="mb-4">
                                        <label class="form-label">Google Sheet URL <span class="text-red-600">*</span></label>
                                        <input type="url"
                                               name="sheet_url"
                                               class="form-input"
                                               placeholder="https://docs.google.com/spreadsheets/d/..."
                                               x-bind:required="selectedAction === 'complete' && submissionType === 'sheet_link'">
                                        <p class="text-sm text-gray-500 mt-1">Paste the full Google Sheets URL</p>
                                    </div>
                                </div>
                        ...
                        {% elif task.submission_type == 'sheet_link' %}
                            <div class="bg-green-50 border border-green-200 rounded-lg p-3">
                                <div class="flex items-center mb-2">
                                    ...
                                    <p class="text-sm font-medium text-green-900">Google Sheet</p>
                                </div>
                ...
                <div id="content-requirements"
                    ...
                    <div>
                        ...
                        <div class="space-y-3">
                            ...
                            <div>
                                <p class="text-xs font-medium text-gray-600 mb-0.5">Category Type</p>
                                <p class="text-sm text-gray-900">{{ task.content_data.category_type }}</p>
                            </div>
                            {% endif %}

                            {% if task.content_data.get('keyword') %}
                            <div>
                                <p class="text-xs font-medium text-gray-600 mb-0.5">Keyword</p>
                                <p class="text-sm text-gray-900">{{ task.content_data.keyword }}</p>
                            </div>
                            {% endif %}

                            {% if task.content_data.get('search_volume') %}
                            <div>
                                <p class="text-xs font-medium text-gray-600 mb-0.5">Search Volume</p>
                                <p class="text-sm text-gray-900">{{ task.content_data.search_volume }}</p>
                            </div>
                            {% endif %}
                            ...
                            {% if task.content_data.get('internal_linking_keywords') %}
                            <div>
                                <p class="text-xs font-medium text-gray-600 mb-0.5">Internal Linking Keywords</p>
                                <p class="text-sm text-gray-900 whitespace-pre-wrap">{{ task.content_data.internal_linking_keywords }}</p>
                            </div>
                            {% endif %}

                            {% if task.content_data.get('internal_link_urls') %}
                            <div>
                                <p class="text-xs font-medium text-gray-600 mb-0.5">Internal Link URLs</p>
                                <p class="text-sm text-gray-900 whitespace-pre-wrap">{{ task.content_data.internal_link_urls }}</p>
                            </div>
                            {% endif %}
                        </div>
                    </div>
                    ...
                    <div>
                        ...
                        <div class="space-y-3">
                            ...
                            {% if task.content_data.get('meta_title') %}
                            <div>
                                <p class="text-xs font-medium text-gray-600 mb-0.5">Meta Title</p>
                                <p class="text-sm text-gray-900">{{ task.content_data.meta_title }}</p>
                            </div>
                            {% endif %}
...
# Write to file
with open('app/templates/task_detail.html', 'w', encoding='utf-8') as f:
    f.write(template_content)

print("Template fixed successfully!")
print("- Content Requirements section moved to sidebar")
print("- Made collapsible with localStorage persistence")
print("- Added keyboard accessibility")
print("- Default state: collapsed")
