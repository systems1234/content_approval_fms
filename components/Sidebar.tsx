...
export default function Sidebar({ user, users, stageCounts, canApprove, isAdmin }: Props) {
  ...
  return (
    <aside className={`sidebar${collapsed ? ' collapsed' : ''}`}>
      ...
      <Link href="/dashboard" className="sidebar-brand">
        ...
        <div>
          <b>content</b><span style={{ marginLeft: 4 }}>fms</span>
        </div>
      </Link>
      ...
      <div className="sidebar-scroll">
        ...
        {stages.map(s => (
          <Link key={s.n} href={s.href} className={`nav-item${isActive(s.href) ? ' active' : ''}`}>
            ...
            {s.count > 0 && (
              <span className={`nc${[2, 4].includes(s.n) && s.count > 0 ? ' live' : ''}`}>{s.count}</span>
            )}
          </Link>
        ))}
        ...
        {/* Actor switcher — visible only when no real auth */}
        {users.length > 1 && (
          <>
